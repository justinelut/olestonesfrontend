...
export default function Login() {
  ...
  if (isAuthenticated()) {
    if (redirect) {
      return <Navigate to={`/products/${redirect}`} />;
    }
  }

  const onSubmit = async (data) => {
    const results = await sender('/api/users/login', {
      email: data.email,
      password: data.password,
    });
    if (results.status === 401) {
      setErrMessage(results.data.errors[0].message);
    } else {
      signIn({
        token: results.token,
        expiresIn: results.exp,
        tokenType: 'Bearer',
        authState: results.user,
      });
      setSuccMessage('Login was successful, Redirecting...');
